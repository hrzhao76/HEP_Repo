...
def get_gbdt_model(n_estimators, learning_rate, max_depth):
...
def train(sample_path, clf_model, training_vars, training_weights, output_path):
    ...
    if isinstance(clf_model, GradientBoostingClassifier):
        ...
        y_test_decisions = clf_model.decision_function(X_test[training_vars])
...
def main(output_path, sample_path, training_vars, training_weights, **kwargs):
    ...
    pickle.dump(clf_model, open(os.path.join(output_path, "clf.pkl"), 'wb'))
