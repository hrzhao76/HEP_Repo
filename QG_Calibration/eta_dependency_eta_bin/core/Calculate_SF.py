...
import pickle
import joblib
from .utils import HistBins, label_pt_bin, label_eta_bin
...
def Plot_ROC(Extraction_Results, output_path, period, reweighting_var, reweighting_option):
    ...
    label_var = ['jet_nTracks', 'jet_trackBDT', 'GBDT_newScore']
    label_keys = [*Extraction_Results['jet_nTracks'][0]]
    for l_ptrange in label_eta_bin[:-1]:
...
def Plot_Fraction(Extraction_Results, output_path, period, reweighting_var, reweighting_option):
    ...
    fractions = fraction_pt_slices.reshape((4, 4)).swapaxes(0,1) # for the eta dependence study
...
def Plot_Parton_ForwardvsCentral(pt, var, output_path, period, reweighting_var, reweighting_option, p_Forward_Quark, p_Central_Quark, p_Forward_Gluon, p_Central_Gluon ):
    ...
    ax0.legend()
    ampl.draw_atlas_label(0.1, 0.85, ax=ax0, energy="13 TeV")
    ax0.set_title(f"{pt} GeV: MC Q/G in eta region " + rf"{Map_var_title[var]}, {reweighting_option}")
...
def Plot_Extracted_unumpy(pt, var, output_path, period, reweighting_var, reweighting_factor, p_Quark, extract_p_Quark, p_Gluon, extract_p_Gluon, extract_p_Quark_Data, extract_p_Gluon_Data,
    ...
    for i, jet_type in enumerate(jet_types):  # i is the idx of jet type
        ...
        ax0.errorbar(x = bin_centers, y = plot_data_bin_content[i][0], yerr = plot_data_bin_error[i][0], drawstyle = 'steps-mid', label = f"Truth MC, values:{np.sum(plot_data_bin_content[i][0]):.3e}, variances:{np.sum(plot_data_bin_error[i][0] ** 2):.3e}")
        ax0.errorbar(x = bin_centers, y = plot_data_bin_content[i][1], yerr = plot_data_bin_error[i][1], drawstyle = 'steps-mid', label = f"Extracted MC, values:{np.sum(plot_data_bin_content[i][1]):.3e}, variances:{np.sum(plot_data_bin_error[i][1] ** 2):.3e}")
        ax0.errorbar(x = bin_centers, y = plot_data_bin_content[i][2], yerr = plot_data_bin_error[i][2], drawstyle = 'steps-mid', label = f"Extracted Data, values:{np.sum(plot_data_bin_content[i][2]):.3e}, variances:{np.sum(plot_data_bin_error[i][2] ** 2):.3e}", color= "black", linestyle='', marker= "o")
...
def Plot_WP(WP, var, output_path, period, reweighting_var, reweighting_factor,
    ...
    bin_edges = np.array(label_eta_bin)
    bin_centers = 1/2 * (bin_edges[:-1] + bin_edges[1:])
    ...
    ax1.set_ylim(0.7, 1.3)
    ax1.set_xlim(bin_edges[0], bin_edges[-1])
    ax1.set_xlabel(f"{Map_var_title['jet_eta']}")
...
def Calculate_SF(input_mc_path, input_data_path, period, reweighting_factor, output_path):
    ...
    for reweighting_var in ["ntrk", "bdt", "newBDT"]:
        ...
        for var in label_var:
            ...
            for WP in WPs:
                ...
                for ii, l_pt in enumerate(label_eta_bin[:-1]):
