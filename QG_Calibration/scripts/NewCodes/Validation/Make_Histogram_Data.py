...
def Make_Histogram_Data(sample_folder_path, period, output_path,slice, reweighting_file_path):
    if not period in ['15', '16', '17', '18']:
        raise Exception(f"Period {period} not in supported periods. Currently supported: ['15', '16', '17', '18']")

    # if period == '1516':
    #     period_data = sorted(sample_folder_path.rglob(f"*data{period[:2]}_13TeV.periodD.physics_Main_minitrees.root")) + sorted(sample_folder_path.rglob(f"*data{period[2:]}_13TeV.periodZ.physics_Main_minitrees.root"))
    #     # Debug use
    ...
    # else:
    period_data = sorted(sample_folder_path.rglob(f"*data{period}_13TeV.period{slice}.physics_Main_minitrees.root"))

    output_file_name = output_path.as_posix() + f"/dijet_data_NEW_{period}{slice}"
...
if __name__ == '__main__':
    ...
    period = args.period
    slice = args.slice
    output_path = Path(f"New_Codes/Validation/CheckData{period}{slice}")
    ...
    Make_Histogram_Data(sample_folder_path=minitrees_folder_path, period=period, output_path = output_path, slice = slice, reweighting_file_path = None)
