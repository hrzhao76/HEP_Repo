...
def Get_ReweightingFactor(input_file: str, reweight_var: str ):
    ...
    for l_pt_bin in label_pt_bin[:-1]:
        ...
        if np.sum(Forward_Quark)==0:
            quark_factor[str(l_pt_bin)] = np.ones(Forward_Quark)
        else: 
            quark_factor[str(l_pt_bin)] = safe_array_divide(p_H_Q, p_L_Q)        
        if np.sum(Forward_Gluon)==0:
            gluon_factor[str(l_pt_bin)] = np.ones(Forward_Gluon)
        else:    
            #quark_factor[str(l_pt_bin)] = safe_array_divide(p_H_Q, p_L_Q) 
...
def Make_Histogram_Pythia(sample_folder_path, period, sum_of_weights, output_path, mc_syst_type, tree):
    ...
    HistMap = Minitree2Hist(input_folder = period_JZslice, sum_of_weights = sum_of_weights, reweight_var = None, reweight_factor = None, mc_syst_type = mc_syst_type, tree = tree)  
...
if __name__ == '__main__':
    ...
    output_path = output_path / f"tree_{tree}" / mc_syst_type
