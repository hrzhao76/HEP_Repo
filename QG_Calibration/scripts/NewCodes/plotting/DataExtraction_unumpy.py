...
def Plot_ForwardCentral(pt, var, output_path, period, required_var, reweighting_factor, p_Forward_Quark, p_Central_Quark, p_Forward_Gluon, p_Central_Gluon ):
    ...
    ax0.set_title(f"{pt} GeV: MC Q/G in eta region " + rf"{Map_var_title[var]}"  + f" distribution, {reweighting_factor} reweighting")
    ...
    ax1.plot()
    output_path_new = output_path / period / f"{required_var}_{reweighting_factor}" / var 
    if not output_path_new.exists():
        output_path_new.mkdir(parents = True, exist_ok =True)
    fig.savefig(output_path_new / f"MC_truth_Q_G_{pt}_{var}_{reweighting_factor}_reweighting.jpg", dpi=100)
    plt.close()
...
def Plot_Extracted_unumpy(pt, var, output_path, period, reweighting_var, reweighting_factor, p_Quark, extract_p_Quark, p_Gluon, extract_p_Gluon, extract_p_Quark_Data, extract_p_Gluon_Data,
    ...
    for i, jet_type in enumerate(jet_types):  # i is the idx of jet type
        ...
        fig.tight_layout()
        fig.savefig( output_path_new / f"DataExtraction_extracted_{pt}_{jet_type}_{var}.jpg", dpi=100)
        plt.close()
...
def DataExtraction(input_mc_path, input_data_path, period, reweighting_factor, output_path):
        ...
        for var in label_var:
            for l_pt in label_ptrange[:-1]:
                ...
                Plot_Extracted_unumpy(pt = l_pt, var= var, output_path= output_path, 
                                ...
                                extract_p_Quark_Data = extract_p_Quark_Data, extract_p_Gluon_Data = extract_p_Gluon_Data,
                                show_yields=True, n_Forward_MC = n_Forward_MC, n_Central_MC = n_Central_MC, 
                                n_Forward_Data = n_Forward_Data, n_Central_Data = n_Central_Data)

                # breakpoint()

    # for inspect_var in ["ntrk"]:
...
if __name__ == '__main__':
    ...
    DataExtraction(input_mc_path=mc_file_path, input_data_path=data_file_path, period = period, reweighting_factor = args.reweighting , output_path = output_path)
