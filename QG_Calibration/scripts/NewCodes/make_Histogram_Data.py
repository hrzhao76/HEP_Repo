...
def Minitree2Hist_Data(input_folder, period):
    ...
    for i, dir in enumerate(input_folder):
        ...
        for file in sorted(dir.glob("*.minitrees.root")):
            ...
            HistMap = Split_jets_Data(HistMap = HistMap, dijets_array = dijets)
...
def Split_jets_Data(HistMap, dijets_array):
    n_jets = 0
...
def Make_Histogram_Data(sample_folder_path, period, output_path):
...
if __name__ == '__main__':
    ...
    Make_Histogram_Data(sample_folder_path=minitrees_folder_path, period=period, output_path = output_path)
