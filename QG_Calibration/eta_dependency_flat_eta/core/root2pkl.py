...
import re
from .utils import check_inputpath, check_outputpath, logging_setup, eta_bins
...
def root2pkl(root_file_path, is_MC=True, output_path=None, 
    ...
    #### do the eta flatten based on pt 
    if not eta_reweight_factor is None:
        empty_list = []
        for pt_idx, pt in enumerate(label_pt_bin[:-1]):
            pythia_period_pt = sample_pd_label.loc[sample_pd_label['pt_idx']==pt_idx]
            eta_idx  = np.digitize(pythia_period_pt['jet_eta'], bins=eta_bins) - 1

            pythia_period_pt['original_event_weight'] = pythia_period_pt['event_weight']

            pythia_period_pt['event_weight'] = pythia_period_pt['event_weight'] * eta_reweight_factor[pt][eta_idx]
            empty_list.append(pythia_period_pt)
        
        sample_pd_label = pd.concat(empty_list)
    if is_MC:
