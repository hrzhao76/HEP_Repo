...
workdir=/global/cfs/projectdirs/atlas/hrzhao/qgcal/BDT_EPEML/scripts
outputdir=/global/cfs/projectdirs/atlas/hrzhao/qgcal/BDT_EPEML/perpared_dijets_w_etalabel
sampledir=/global/cfs/projectdirs/atlas/hrzhao/qgcal/Samples_New

period=${1}
tree=nominal

python -u ${workdir}/prepare_training_w_etalabel.py --path ${sampledir}/pythia/pythia${period} --period ${period} --output-path ${outputdir}/Processed_Samples_Pythia_Nov8/period${period} --tree ${tree} | tee ${workdir}/log/log.prepare_training_w_etalabel.Pythia.period${period}.txt
