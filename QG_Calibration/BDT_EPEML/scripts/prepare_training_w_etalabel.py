####### Change based on make_Histogram_Pythia_syst_JES.py
import argparse
import numpy as np
...
def Minitree2Hist(input_folder, sum_of_weights, mc_syst_type, tree, reweight_var = None, reweight_factor = None):
    ...
    for i, dir in enumerate(input_folder):
        ...
        dijet_array_all[f"JZ{JZ_slice_number}"] = []
        for file in sorted(dir.glob("*.minitrees.root")):
            ...
            dijet_array_all[f"JZ{JZ_slice_number}"].append(dijets)
    return HistMap, dijet_array_all
...
def Make_Histogram_Pythia(sample_folder_path, period, sum_of_weights, output_path, mc_syst_type, tree):
    if not period in ["A", "D", "E"]:
        raise Exception(f'Period {period} not in supported periods. Currently supported: ["A", "D", "E"]')
    ...
    output_dijet_array = output_file_name.as_posix() + ".npy"



    HistMap, dijet_array_all = Minitree2Hist(input_folder = period_JZslice, sum_of_weights = sum_of_weights, reweight_var = None, reweight_factor = None, mc_syst_type = mc_syst_type, tree = tree)  
    # np.save(output_dijet_array, dijet_array_all)
    WritePickleFile(dijet_array_all, output_file_name.as_posix() + "event.pkl")
    ...
    # HistMap_ntrk_reweighting_quark_factor, dijet_array_all_ntrk_reweighting_quark_factor = Minitree2Hist(input_folder = period_JZslice, sum_of_weights = sum_of_weights, reweight_var = "ntrk", reweight_factor = ntrk_reweighting_quark_factor, mc_syst_type = mc_syst_type, tree = tree)
    # WriteHistRootFile(HistMap_ntrk_reweighting_quark_factor, output_root_file, TDirectory_name = "ntrk_Reweighting_Quark_Factor")
    # np.save(output_dijet_array[:-4] + "_ntrk_reweighting_quark_factor" + ".npy", dijet_array_all_ntrk_reweighting_quark_factor)

    # HistMap_ntrk_reweighting_gluon_factor, dijet_array_all_ntrk_reweighting_gluon_factor = Minitree2Hist(input_folder = period_JZslice, sum_of_weights = sum_of_weights, reweight_var = "ntrk", reweight_factor = ntrk_reweighting_gluon_factor, mc_syst_type = mc_syst_type, tree = tree)
    # WriteHistRootFile(HistMap_ntrk_reweighting_gluon_factor, output_root_file, TDirectory_name = "ntrk_Reweighting_Gluon_Factor")
    # np.save(output_dijet_array[:-4] + "_ntrk_reweighting_gluon_factor" + ".npy", dijet_array_all_ntrk_reweighting_quark_factor)

    #HistMap_bdt_reweighting_quark_factor = Minitree2Hist(input_folder = period_JZslice, sum_of_weights = sum_of_weights, reweight_var = "bdt", reweight_factor = bdt_reweighting_quark_factor, mc_syst_type = mc_syst_type)
    #WriteHistRootFile(HistMap_bdt_reweighting_quark_factor, output_root_file, TDirectory_name = "bdt_Reweighting_Quark_Factor")
    
    #HistMap_bdt_reweighting_gluon_factor = Minitree2Hist(input_folder = period_JZslice, sum_of_weights = sum_of_weights, reweight_var = "bdt", reweight_factor = bdt_reweighting_gluon_factor, mc_syst_type = mc_syst_type)
...
if __name__ == '__main__':
    ...
    if sum_of_weights_file_path.exists():
        ...
        np.save(sum_of_weights_file_path, sum_of_weights)

    output_path = output_path 
    if not (output_path).exists():
        output_path.mkdir(parents=True)

    Make_Histogram_Pythia(sample_folder_path=minitrees_folder_path, period=period, sum_of_weights=sum_of_weights, output_path = output_path, mc_syst_type = mc_syst_type, tree = tree)
